...
namespace SQLite.Designer.Design
{
    ...
    internal class Table : ViewTableBase, ICustomTypeDescriptor
    {
        ...
        internal Table( string schema, string tableName, DbConnection connection, TableDesignerDoc owner )
        {
            _owner = owner;
            _oldname = tableName;
            ...
            if ( _exists )
            {
                //TODO: Implement foreign key retrieval
                //using ( DataTable tbl = connection.GetSchema( "ForeignKeys", new string[] { Catalog, _schema, Name } ) )
                //{
            ...
            using ( DataTable tbl = connection.GetSchema( "Columns", new string[] { Catalog, _schema, Name } ) )
            {
                foreach ( DataRow row in tbl.Rows )
                {
        ...
        {
            using ( DataTable tbl = _connection.GetSchema( "Tables", new string[] { Catalog, Schema, Name } ) )
            {
                if ( tbl.Rows.Count > 0 )
                {
                    _exists = true;
                    //_origSql = tbl.Rows[ 0 ][ "TABLE_DEFINITION" ].ToString().Trim().TrimEnd( ';' );
                    _origSql = string.Empty;//Does it makes any sense to have this here?
                    _oldname = Name;
                }
                else
                {
            ...
            using ( DataTable tbl = NpgsqlObjectSelector.GetSchemaTable( "Indexes", cmd, new object[] { Catalog, Schema, Name } ) )
            {
                foreach ( DataRow row in tbl.Rows )
                {
                    if ( ( bool ) row[ "IsPrimary" ] == false )
                    {
                        if ( row[ "Name" ].ToString().StartsWith( "pg_", StringComparison.OrdinalIgnoreCase ) == false )
                        {
            ...
            if ( !string.IsNullOrWhiteSpace( _origSql ) )
            {
            ...
            //TODO: Implement trigger retrieval
            //
            //using ( DataTable tbl = _connection.GetSchema( "Triggers", new string[] { Catalog, _schema, Name } ) )
            //{
            //    foreach ( DataRow row in tbl.Rows )
            //    {
        ...
        {
            return String.Format( CultureInfo.InvariantCulture, "\"{0}\".\"{1}\".\"{2}\"", Catalog, Schema, Name );
        }
        ...
        public string GetSql()
        {
            ...
            if ( altName != null )
            {
                ...
                builder.AppendFormat( "INSERT INTO \"{0}\".\"{1}\" (", Schema, Name );
                foreach ( Column c in Columns )
                {
